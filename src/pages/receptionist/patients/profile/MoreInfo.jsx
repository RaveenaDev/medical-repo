                ...
                {items.map((item, i) => {
                    ...
                    return (
                        <Box component="li" key={i} sx={{ mb: 1 }}>
                            {isImage && (
                                <Box
                                    onClick={() => handlePreviewOpen(item.url, item.fileType, item.filename)}
                                    sx={{
                                        cursor: "pointer",
                                        "&:hover": { opacity: 0.8 }
                                    }}
        ...
        <Box component="ul" sx={{ pl: depth ? 3 : 2, m: 0 }}>
            {items.map((it, i) => {
                ...
                if (it && typeof it === "object") {
                    ...
                    const opts = it.options || it.choices || it.answers || it.responses || it.items;
                    ...
                    return (
                        <Box component="li" key={i} sx={{ mb: 1 }}>
                            ...
                            <ExtraObjectFields
                                obj={it}
                                excludeKeys={[
                                    "question", "ques", "que", "q", "title", "label",
                                    "answer", "ans", "response", "value",
                                    "options", "choices", "answers", "responses", "items"
                                ]}
                                depth={depth + 1}
                            />
                        </Box>
...
const ValueNode = ({ label, value, depth = 0 }) => {
    ...
    if (Array.isArray(value)) {
        ...
        const isFileArray = value.some(item => typeof item === 'object' && 'url' in item);
        if (isFileArray) {
        ...
        return (
            <Box component="ul" sx={{
                ...
                {entries.map(([k, v]) => {
                    ...
                    if (Array.isArray(v) && v.some(item => typeof item === 'object' && 'url' in item)) {
                        return (

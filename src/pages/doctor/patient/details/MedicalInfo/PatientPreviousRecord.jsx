...
const PatientPreviousRecord = ({ patientDetails = {}, loading }) => {
  ...
  const renderConsultation = (item) => {
    ...
    const cmList =
        Array.isArray(cmRoot?.currentMedications)
    ...
    const dxBundle = pruneDeep({
      ...
      "Dx Pain": dx?.dxPain?.hasPain ? "Has Pain" : dx?.dxPain?.hasPain === false ? "No Pain" : undefined,
      ...
      Weight: (dx?.weight?.value ? `${dx.weight.value}` : null) && (dx?.weight?.unit ? `${dx.weight.value} ${dx.weight.unit}` : `${dx.weight.value}`),
      Height: (dx?.height?.value ? `${dx.height.value}` : null) && (dx?.height?.unit ? `${dx.height.value} ${dx.height.unit}` : `${dx.height.value}`),
      "Heart Rate": dx?.heartRate,
      "Respiration Rate": dx?.respirationRate,
      "Oxygen Level": dx?.oxygenLevel,
      Temperature: dx?.temperature,
      "BP": (dx?.systolic || dx?.diastolic) ? `${dx?.systolic ?? ""}${dx?.systolic && dx?.diastolic ? "/" : ""}${dx?.diastolic ?? ""}` : undefined,
    });

    // Build list of shown keys to exclude from dynamic section
    const OMIT = new Set([
      "files","images","videos","notes","previousHistoryData",
      "medicalHistory","currentMedications","diagnosisVitals"
    ]);
          ...
          <div className="visit-details">
            ...
            <div className="kv-table">
              ...
              {c?.followUpRequired !== undefined
                  ? renderStack("Follow-up Required", c.followUpRequired ? "Yes" : "No")
                  : null}
              {c?.treatment?.note ? renderStack("Treatment Note", c.treatment.note) : null}
            </div>
            ...
            {(!isEmptyValue(cmFlag) || !isEmptyValue(cmList)) && (
                <>
                  ...
                  <div className="kv-table">
                    {!isEmptyValue(cmFlag) && renderStack("Current Medication", cmFlag)}
                    {!isEmptyValue(cmList) &&
                        renderStack(
                            "Medication List",
                            cmList.map((m,i) => {

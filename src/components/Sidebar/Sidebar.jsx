...
const Sidebar = ({role}) => {
    ...
    return (
        <div className={styles.sidebar} style={{width:'270px',minWidth:'270px'}}>
            <>
                {
                    sideOptions.map((option, index) => (
                        <div key={index}
                             className={`${styles.sideElement} ${activeIndex === index ? styles.active : ''}`}
            ...
            {activeIndex === sideOptions.findIndex(option => option.title === 'Settings') && (
                <div className={adi.side_panel}>
                    <Box>
                        <Typography variant="body1" onClick={() => handleSubClick('')} gutterBottom sx={{ cursor: 'pointer' }}>
                            FAQ's
                        </Typography>
                        <Typography variant="body1" onClick={() => handleSubClick('privacyPolicy')} gutterBottom sx={{ cursor: 'pointer' }}>
                            Privacy Policy
                        </Typography>
                        <Typography variant="body1" onClick={() => handleSubClick('helpAndSupport')} gutterBottom sx={{ cursor: 'pointer' }}>
                            Help & Support
                        </Typography>
